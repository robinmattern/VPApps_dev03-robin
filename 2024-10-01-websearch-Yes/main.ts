...
// No need for SEARCH_URL env var since we're hardcoding DuckDuckGo
...
async function getNewsUrls(query: string) {
  ...
  const searchResultsJson: DuckDuckGoResponse = await searchResults.json();
  console.log("API Response:", JSON.stringify({
    AbstractURL: searchResultsJson.AbstractURL,
    Results: searchResultsJson.Results,
    RelatedTopics: searchResultsJson.RelatedTopics.slice(0, 3) // Limit for brevity
  }, null, 2));
  ...
  const urls = results.map(result => result.FirstURL).slice(0, 5); // Get up to 5 URLs
  if (urls.length === 0) {
    console.log("No URLs found, returning fallback.");
    return ["https://www.sandyspringsga.gov/"]; // Fallback if no results
  }
